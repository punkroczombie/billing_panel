            ...
            <div class="content-box overflow-hidden">
                ...
                <div>
                    ...
                    @if (count($customConfig) > 0)
                        ...
                        @php
                            function extractOrder($item): int
                            {
                                $name = explode('|', $item->name)[1] ?? $item->name;
                                return (int)explode('|', $item->name)[0];
                            }

                            usort($customConfig, function($a, $b) {
                                return extractOrder($a) <=> extractOrder($b);
                            });
                        @endphp
                    @endif
                    ...
                    @foreach ($customConfig as $cconfig)
                        <h2 class="text-lg font-bold mt-6">{{$name = explode('|', $cconfig->name)[1] ?? $cconfig->name}}</h2>
                        @php
                            $configItems = $cconfig
                                ->configurableOptions()
                                ->orderBy('order', 'asc')
